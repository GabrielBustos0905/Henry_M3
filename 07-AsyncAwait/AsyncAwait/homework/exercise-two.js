...
function problemA () {
  ...
  async function notOrder(){
    await Promise.all([readAsyncFile('poem-two/stanza-01.txt'), readAsyncFile('poem-two/stanza-02.txt')])
    console.log('done')
  }
...
function problemB () {
  ...
  const arrayPromise = filenames.map(fn => {readAsyncFile(fn)})

  async function notOrdered(){
...
function problemD () {
  ...
  async function inOrder(){
    try{
      for(let i = 0; i < filenames.length; i++){
        await asyncReadFile(filenames[i])
      }
    } catch(e){
        magenta(e)
    } finally{
